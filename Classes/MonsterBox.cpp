...
void MonsterBox::drawMonsterBox(class Creature * _creature)
{
    ...
    // Draw HP and My of creature in parameter input
   
  
    spBackGround = Sprite::create("tl.png");
    
    Size target = Size(150, 150);
    Size origin = spBackGround->getContentSize();
    
    spBackGround->setScale(target.width/origin.width,
                           target.height/origin.height);
    
    spBackGround->setContentSize(Size(150, 150));
    spBackGround->setPosition(Point::ZERO);
    ...
    //set lbname
    char txtname[256];
//    sprintf(txtname, "%s", _creature->getName().c_str())
    CCLOG("name of creature: %s", _creature->getName().c_str());
    lbname = Label::createWithTTF(_creature->getName().c_str(), "fonts/MS Gothic.ttf", 30);
//    lbname->setPosition(Point::ZERO);
    lbname->setPosition(this->getContentSize().width/2,
                        this->getContentSize().height - lbname->getContentSize().height/2 - 20);
    ...
    sprintf(txtHp, "HP: %d", _creature->getHp());
    sprintf(txtMp, "MP: %d", _creature->getMp());
    
    lbHP = Label::createWithTTF(txtHp, "fonts/MS Gothic.ttf", 24);
    lbMP = Label::createWithTTF(txtMp, "fonts/MS Gothic.ttf", 24);
    ...
    arr->addObject(lbHP);
    arr->addObject(lbMP);
    CCLOG("CONTENT SIZE %f %f", this->getContentSize().width,
